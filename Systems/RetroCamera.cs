...
public class RetroCamera : MonoBehaviour
{
    ...
    private float _espRefreshTimer = 0f;
    ...
    private bool _hasRunDebugger = false;
    ...
    void Update()
    {
        ...
        EntityList.UpdatePlayers();

        if (ESPOptions.AntiCounter?.Value ?? false)
        {
            CounterDetector.CheckForCounters();
        }

        if (!Core._initialized || !_gameFocused) return;

        var em = EntityList.GetClientWorld().EntityManager;
        var localPlayer = EntityList.GetLocalPlayer();

        _espRefreshTimer += Time.deltaTime;
        if (_espRefreshTimer >= 0.5f)
        {
            EntityList.UpdatePlayers();
            _espRefreshTimer = 0f;
        }
        ...
        if (ESPOptions.ShowContainers?.Value ?? false)
        {
            EntityList.UpdateContainers();
        }

        if (ESPOptions.ShowBloodCarriers?.Value ?? false)
        {
            EntityList.UpdateBloodCarriers();
        }

        if (TweaksOptions.RemoveMinimapFog?.Value ?? false)
        {
            MinimapTweaks.RemoveMinimapFog();
        }

        if (TweaksOptions.RemoveForrestFog?.Value ?? false)
        {
            MinimapTweaks.RemoveFullScreenEffects();
        }

        Entity myPlayer = Entity.Null;
        try
        {
            myPlayer = EntityList.GetLocalPlayer();
        }
        catch (System.Exception ex)
        {
            Core.Log.LogError($"[RetroCamera] Failed to get local player: {ex.Message}");
        }

        if (Input.GetKey(KeyCode.UpArrow))
        {
            CounterDetector.PerformEvade();
        }

        Aimbot.Run();

        if (CounterDetector.ShouldEvade && Time.time <= CounterDetector.EvadeUntilTime)
        {
            bool isMousePressed = Input.GetMouseButton(0);
            bool isQorE = CounterDetector.IsQorEBeingCast();

            if (isMousePressed || isQorE)
            {
                if (isQorE)
                {
                    // Q or E pressed → always trigger external spam
                    CounterDetector.TriggerExternalSpam();
                }
                else
                {
                    // Mouse pressed → handle by config
                    if (ESPOptions.AntiCounterMethod2?.Value ?? false)
                        CounterDetector.PerformEvade();
                    else
                        CounterDetector.TriggerExternalSpam();
                }
    ...
    void OnGUI()
    {
        ...
        //{
        //    CounterDetector.CheckForCounters();
        //    CounterDetector.MonitorStopCondition();
        //}

        Primitives.Init();
        Overlay.DrawOverlay();

        if (!ESPOptions.ESPEnabled?.Value ?? true) return;

        var world = EntityList.GetClientWorld();
        if (world == null) return;

        if (ESPOptions.ShowPlayers?.Value ?? false)
        {
            Render.DrawPlayers();
        }
        ...
        if (ESPOptions.ShowBloodCarriers?.Value ?? false)
        {
            float minQuality = ESPOptions.MinBloodQuality;
            EntityList.BloodCarriers = EntityList.GetBloodCarriers(world, minQuality);
            Render.DrawBloodCarriers();
        }
